...
class BI_LSTM_CRF(object):
    def __init__(self, config,is_crf = True):
        ...
        if self.is_training ==1:
            ...
            # todo: add attention cell-->( attention = softmax(score), score = v * tanh(W_att_vec_size, hi) )
            #lstm_cell_fw = tf.contrib.rnn.AttentionCellWrapper(lstm_cell_fw, attn_length=11)
            #lstm_cell_bw = tf.contrib.rnn.AttentionCellWrapper(lstm_cell_bw, attn_length=11)
...
#在条件随机场中用到的
    ...
    def train(self, sess, save_file, X_train, y_train, X_val, y_val,matchListB,matchListE):
        ...
        # dataPath = '/root/PycharmProjects/Att/bi_LSTM_CRF_test/data/'
                ...
                if iteration % 10 == 0:
                    ...
                    if f1_val > self.max_f1:
                        ...
                        if f1_val>0.85:
                            ...
                            fout = open(dataPath+'validResult.txt','a',encoding='utf-8')
                            fout.write("\nNow the f1_val and the keep_prob is"+str(f1_val)+" "+str(self.keep_prob)+'\n')
                            data_helper.getTestResult(X_val_batch, predicts_val, y_val_batch, fout)
                            fout.close()
    ...
    def test(self, sess, X_test,Y_test,fileName):
        ...
        for i in range(num_iterations):
            ...
            if i == num_iterations - 1 and len(X_test_batch) < self.batch_size:
                ...
                data_helper.getTestResult(X_test_batch, predicts, [], fout)
            else:
                X_test_batch = np.array(X_test_batch)
                length, max_scores, max_scores_pre = sess.run([self.length, self.max_scores, self.max_scores_pre],
                                                              feed_dict={self.inputs: X_test_batch})
